...
router.post('/analyze-deal', verifyToken, async (req, res) => {
    try {
        ...
        if (!budget.allowed) {
            aiLogger.warn("Deal Intelligence Quota Rejected", { organization_id, reason: budget.reason });
        ...
        if (cached) {
            aiLogger.info("Deal Analysis Cache Hit", { organization_id, dealId: dealData.id || 'new' });
            return res.json({ source: 'cache', data: cached });
        }

        aiLogger.debug("Deal Analysis Requested (Cache Miss)", { organization_id, dealId: dealData.id || 'new' });
...
"dealScore" (1-100), "liquidity" ("HIGH", "MODERATE", "LOW"), "closeProbability" (0-100), "suggestedFee" (string, e.g. "$10k-$15k").`;
        ...
        aiLogger.error("Deal Analysis Gateway Failed", { error: error.message, stack: error.stack });
...
router.post('/oscar', verifyToken, async (req, res) => {
    try {
        ...
        aiLogger.debug("OSCAR Agent Invoked", { organization_id, context: contextContext });
...
Keep your responses helpful, concise, and focused strictly on Real Estate Wholesaling or WholesaleOS platform usage. Do not answer questions outside of this scope. Use Markdown.`;
        ...
        aiLogger.error("OSCAR Gateway Failed", { error: error.message, stack: error.stack });
