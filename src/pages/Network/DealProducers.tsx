            ...
            {producers.map((producer, i) => (
                <div key={producer.wholesaler_id} className="relative p-6 bg-slate-900/50 rounded-xl border border-slate-700/50 hover:border-emerald-500/30 transition-colors overflow-hidden group">
                    ...
                    <div className="relative z-10 w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                        <div
                            ref={(el) => { if (el) el.style.width = `${producer.avg_deal_score}%`; }}
                            className={`h-full rounded-full transition-all duration-1000 ${producer.avg_deal_score >= 85 ? 'bg-gradient-to-r from-emerald-500 to-teal-400' : producer.avg_deal_score >= 70 ? 'bg-gradient-to-r from-blue-500 to-indigo-400' : 'bg-slate-500'}`}
                        />
                    </div>
                </div>
            ))}
